          ...
          <div v-if="modelValue" class="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg z-10 overflow-hidden">
            ...
            <div class="p-6 space-y-5">
              ...
              <div>
                ...
                >
                <p class="mt-1.5 text-[11px] text-gray-400">Leave blank to auto-assign order at the end.</p>
              </div>
            ...
            <div class="flex items-center justify-end gap-3 px-6 pb-6">
              ...
              <button id="module-submit-btn" @click="handleSubmit" :disabled="submitting" class="px-6 py-2.5 text-[13px] font-semibold text-white bg-[#0A733F] rounded-xl hover:bg-[#085a31] active:scale-95 transition-all flex items-center gap-2 disabled:opacity-60 shadow-sm shadow-[#0A733F]/30">
...
const form = reactive({ title: '', content: '', order: null })
...
async function handleSubmit() {
  ...
  submitError.value = ''
  try {
    const payload = { title: form.title.trim(), content: form.content.trim(), order: form.order }
    if (isEditing.value) {
